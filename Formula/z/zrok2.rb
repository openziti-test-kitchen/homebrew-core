class Zrok2 < Formula
  desc "Geo-scale, next-generation sharing platform built on top of OpenZiti (v2)"
  ...
  test do
    ...
    version_output = shell_output("#{bin}/zrok2 version")
    assert_match(/\bv#{version}\b/, version_output)
    assert_match(/[[a-f0-9]{40}]/, version_output)

    status_output = shell_output("#{bin}/zrok2 controller validate #{testpath}/ctrl.yml 2>&1")
